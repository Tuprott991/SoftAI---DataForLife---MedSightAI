...
async def generate_embeddings(
    ...
    try:
        ...
        # Get AI result for text embedding
        # Use only AI predicted diagnosis and concepts
        text_parts = []
        
        ai_result = crud_ai_result.get_by_case(db, case_id=case_id)
        if ai_result:
            if ai_result.predicted_diagnosis:
                text_parts.append(f"Diagnosis: {ai_result.predicted_diagnosis}")
            
            # Extract concepts from concepts JSONB field
            if ai_result.concepts:
                # concepts can be a list of strings or a dict with concept labels
                if isinstance(ai_result.concepts, list):
                    text_parts.append(f"Concepts: {', '.join(ai_result.concepts)}")
                elif isinstance(ai_result.concepts, dict):
                    # If concepts is a dict, try to extract labels
                    labels = ai_result.concepts.get('labels', [])
                    if labels:
                        text_parts.append(f"Concepts: {', '.join(labels)}")
