...
export default function LoginScreen() {
  ...
  const { sendOTP, verifyOTP, isLoading, error, setError, mockOtp } = useAuth();
  ...
  const handleSendOTP = async () => {
    try {
      ...
      // If this is a mock verification (development), auto-verify after 4s WITHOUT showing OTP screen
      if (verificationId && verificationId.startsWith && verificationId.startsWith('mock-')) {
        Alert.alert('Thành công', 'Đang xác thực tự động...');
        ...
        autoFillTimeoutRef.current = setTimeout(async () => {
          // Use the OTP generated by AuthContext (mockOtp)
          const otpToUse = mockOtp ?? Array.from({ length: 6 }, () => Math.floor(Math.random() * 10).toString()).join('');
          console.log('LoginScreen: Auto-verifying with mock OTP:', otpToUse);

          // Call verifyOTP directly to perform mock login (no UI change needed)
          try {
            await verifyOTP(otpToUse);
        ...
        // Real OTP flow: show the OTP input screen
        setStep('otp');
