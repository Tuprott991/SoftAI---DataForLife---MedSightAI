...
export default function LoginScreen() {
  ...
  useEffect(() => {
    return () => {
      if (autoFillTimeoutRef.current) {
        clearTimeout(autoFillTimeoutRef.current as unknown as number);
  ...
  const handleSendOTP = async () => {
    try {
      ...
      const verificationId = await sendOTP(phoneNumber, verifier);
      
      console.log('LoginScreen: OTP sent successfully');

      // Always show the OTP input screen first
      setStep('otp');
      Alert.alert('Thành công', 'Mã OTP đã được gửi!');

      // If this is a mock verification (development), auto-fill and verify after 4s
      if (verificationId && verificationId.startsWith && verificationId.startsWith('mock-')) {
        ...
        autoFillTimeoutRef.current = setTimeout(async () => {
          // Generate random 6-digit OTP (or use test OTP for actual verification)
          const randomOtp = Array.from({ length: 6 }, () => Math.floor(Math.random() * 10).toString()).join('');
          console.log('LoginScreen: Auto-filling random OTP (display only):', randomOtp);

          // Fill all OTP boxes at once with the random OTP for visual effect
          setOtp(randomOtp.split(''));
          
          // Wait 1 second to show the filled OTP, then verify with the correct test OTP
          setTimeout(async () => {
            const otpToVerify = '123456'; // This is what AuthContext expects
            console.log('LoginScreen: Auto-verifying with test OTP:', otpToVerify);
            try {
              await verifyOTP(otpToVerify, verificationId);
              console.log('LoginScreen: Mock auto-verification successful');
            } catch (err) {
              console.error('LoginScreen: Auto-verify failed:', err);
              Alert.alert('Lỗi', 'Xác thực tự động thất bại');
            }
          }, 1000);
